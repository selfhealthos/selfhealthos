...
import type { FitnessPlaylistDetail, FitnessSession, FitnessStats } from "@/lib/api/types";
...
export default async function WorkoutPlaylistPage({
  ...
  try {
    playlist = await serverGet<FitnessPlaylistDetail>(`/fitness/playlists/${key}`);
  } catch (error) {
  ...
  return <WorkoutPlayer playlist={playlist} initialStats={stats} initialRecent={recent} />;
}
