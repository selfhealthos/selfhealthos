...
Before pushing: the checks CI runs are worth running locally first (`ruff check`/`format`, `pytest`, `manage.py makemigrations --check --dry-run`, frontend `typecheck`/`build`) — pushing something CI immediately fails on isn't a real checkpoint.
...
- **Ownership, not real multi-tenancy.** Every domain row is scoped to the user who created it (`created_by` via `OwnedModel`). There's no cross-user sharing model in v1 — don't build features that assume one.
