...
interface BodyMeasurementDao {
    @Query("SELECT * FROM body_measurements WHERE deletedAt IS NULL ORDER BY timestamp DESC")
    fun getAll(): Flow<List<BodyMeasurement>>

    @Query("SELECT * FROM body_measurements WHERE timestamp >= :startMs AND timestamp < :endMs AND deletedAt IS NULL ORDER BY timestamp ASC")
    fun getEntriesForDay(startMs: Long, endMs: Long): Flow<List<BodyMeasurement>>

    @Query("SELECT * FROM body_measurements WHERE deletedAt IS NULL ORDER BY timestamp ASC")
    suspend fun getAllSnapshot(): List<BodyMeasurement>
    ...
    suspend fun getUnsynced(): List<BodyMeasurement>

    @Query("SELECT COUNT(*) FROM body_measurements WHERE isSynced = 0")
     ...
     */
    @Query("UPDATE body_measurements SET deletedAt = :at, updatedAt = :at, isSynced = 0 WHERE id = :id")
     ...
     */
    @Query("DELETE FROM body_measurements WHERE deletedAt IS NOT NULL AND isSynced = 1")
